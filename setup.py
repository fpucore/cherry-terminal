#!/usr/bin/env python
# License: GPL v3 Copyright: 2026, Chris McGimpsey-Jones <chrisjones.unixmen@gmail.com>
...
version = tuple(
    map(
        ...
        re.search(  # type: ignore
            r"^version: Version = Version\((\d+), (\d+)\)", constants, re.MULTILINE
        ).group(1, 2)
    )
)
...
def build_launcher(args: Options, launcher_dir: str = '.', bundle_type: str = 'source') -> str:
    ...
    dest = kitty_exe = os.path.join(launcher_dir, 'cherry-terminal')
...
TryExec=cherry-terminal
StartupNotify=true
Exec=cherry-terminal
Icon=kitty
...
''')
    with open(os.path.join(deskdir, 'cherry-terminal-open.desktop'), 'w') as f:
        f.write(
            '''\
[Desktop Entry]
Version=1.0
Type=Application
Name=Cherry Terminal+ URL Launcher
GenericName=Terminal emulator
Comment=Open URLs with Cherry Terminal+
StartupNotify=true
TryExec=cherry-terminal
Exec=cherry-terminal +open %U
Icon=kitty
Categories=System;TerminalEmulator;
NoDisplay=true
...
def macos_info_plist(for_quake: str = '') -> bytes:
    ...
    pl = dict(
        ...
        CFBundleGetInfoString='Cherry Terminal+ - The fast, feature-rich, GPU based terminal emulator.',
...
def create_minimal_macos_bundle(args: Options, launcher_dir: str, relocate: bool = False) -> None:
    ...
    if relocate:
        shutil.copy2(os.path.join(launcher_dir, "cherry-terminal"), bin_dir)
