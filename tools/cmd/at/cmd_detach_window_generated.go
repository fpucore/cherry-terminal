...
type options_detach_window_type struct {
	Match string
TargetTab string
Self bool
StayInTab bool
...
var options_detach_window options_detach_window_type

type detach_window_json_type struct {
	Match escaped_string`json:"match,omitempty"`
Target_tab escaped_string`json:"target_tab,omitempty"`
Self bool`json:"self,omitempty"`
Stay_in_tab bool`json:"stay_in_tab,omitempty"`
}

func create_payload_detach_window(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := detach_window_json_type{}
	if len(args) != 0 { return fmt.Errorf("%s", "Unknown extra argument(s) supplied to detach_window") }
payload.Match = escaped_string(options_detach_window.Match)
payload.Target_tab = escaped_string(options_detach_window.TargetTab)
payload.Self = options_detach_window.Self
payload.Stay_in_tab = options_detach_window.StayInTab
	io_data.rc.Payload = payload
	return
}

func create_rc_detach_window(args []string) (*utils.RemoteControlCmd, error) {
...
func run_detach_window(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_detach_window)
	if err != nil {
		return
	}

	rc, err := create_rc_detach_window(args)
	if err != nil {
		return
	}
	...
	err = create_payload_detach_window(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_detach_window(parent *cli.Command) *cli.Command {
	ans := parent.AddSubCommand(&cli.Command{
		Name:             "detach-window",
		Usage:            "",
		ShortDescription: "Detach the specified windows and place them in a different/new tab",
		HelpText:         "Detach the specified windows and either move them into a new tab, a new OS window or add them to the specified tab. Use the special value :code:`new` for :option:`kitten @ detach-window --target-tab` to move to a new tab. If no target tab is specified the windows are moved to a new OS window.",
		Run:              run_detach_window,
	})
...
ans.Add(cli.OptionSpec{
            ...
            Type: "bool-set",
            Dest: "Self",
            Help: "Detach the window this command is run in, rather than the active window.",
        })
ans.Add(cli.OptionSpec{
            Name: "--stay-in-tab",
            Type: "bool-set",
            Dest: "StayInTab",
            Help: "Keep the focus on a window in the currently focused tab after moving the specified windows.",
        })
	return ans
}

func init() {
	register_at_cmd(setup_detach_window)
}
