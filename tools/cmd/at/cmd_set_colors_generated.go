...
type options_set_colors_type struct {
	All bool
Configured bool
Reset bool
Match string
...
var options_set_colors options_set_colors_type

type set_colors_json_type struct {
...
func create_payload_set_colors(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := set_colors_json_type{}
	payload.Colors, err = parse_colors_and_files(args)
if err != nil { return err }
payload.Match_tab = escaped_string(options_set_colors.MatchTab)
payload.All = options_set_colors.All
payload.Configured = options_set_colors.Configured
payload.Reset = options_set_colors.Reset
payload.Match_window = escaped_string(options_set_colors.Match)
	io_data.rc.Payload = payload
	return
}

func create_rc_set_colors(args []string) (*utils.RemoteControlCmd, error) {
...
func run_set_colors(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_set_colors)
	if err != nil {
		return
	}

	rc, err := create_rc_set_colors(args)
	if err != nil {
		return
	}
	...
	err = create_payload_set_colors(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_set_colors(parent *cli.Command) *cli.Command {
	ans := parent.AddSubCommand(&cli.Command{
		Name:             "set-colors",
		Usage:            " COLOR_OR_FILE ...",
		ShortDescription: "Set terminal colors",
		HelpText:         "Set the terminal colors for the specified windows/tabs (defaults to active window). You can either specify the path to a conf file (in the same format as :file:`kitty.conf`) to read the colors from or you can specify individual colors, for example::\n\n    kitten @ set-colors foreground=red background=white",
		Run:              run_set_colors,
	})
...
ans.Add(cli.OptionSpec{
            ...
            Type: "bool-set",
            Dest: "All",
            Help: "By default, colors are only changed for the currently active window. This option will cause colors to be changed in all windows.",
        })
ans.Add(cli.OptionSpec{
            Name: "--configured -c",
            Type: "bool-set",
            Dest: "Configured",
            Help: "Also change the configured colors (i.e. the colors kitty will use for new windows or after a reset).",
        })
ans.Add(cli.OptionSpec{
            Name: "--reset",
            Type: "bool-set",
            Dest: "Reset",
            Help: "Restore all colors to the values they had at kitty startup. Note that if you specify this option, any color arguments are ignored and :option:`kitten @ set-colors --configured` and :option:`kitten @ set-colors --all` are implied.",
        })
ans.Add(cli.OptionSpec{
            Name: "--match -m",
            Type: "",
...
func init() {
	register_at_cmd(setup_set_colors)
}
