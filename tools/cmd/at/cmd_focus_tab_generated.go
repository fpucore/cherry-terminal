...
type options_focus_tab_type struct {
	Match string
}

var options_focus_tab options_focus_tab_type

type focus_tab_json_type struct {
	Match escaped_string`json:"match,omitempty"`
}

func create_payload_focus_tab(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := focus_tab_json_type{}
	if len(args) != 0 { return fmt.Errorf("%s", "Unknown extra argument(s) supplied to focus_tab") }
payload.Match = escaped_string(options_focus_tab.Match)
	io_data.rc.Payload = payload
	return
}

func create_rc_focus_tab(args []string) (*utils.RemoteControlCmd, error) {
...
func run_focus_tab(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_focus_tab)
	if err != nil {
		return
	}

	rc, err := create_rc_focus_tab(args)
	if err != nil {
		return
	}
	...
	err = create_payload_focus_tab(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_focus_tab(parent *cli.Command) *cli.Command {
	ans := parent.AddSubCommand(&cli.Command{
		Name:             "focus-tab",
		Usage:            "",
		ShortDescription: "Focus the specified tab",
		HelpText:         "The active window in the specified tab will be focused.",
		Run:              run_focus_tab,
	})
...
func init() {
	register_at_cmd(setup_focus_tab)
}
