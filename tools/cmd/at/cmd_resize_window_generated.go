...
type options_resize_window_type struct {
	Match string
Increment int
Axis string
Self bool
...
var options_resize_window options_resize_window_type

type resize_window_json_type struct {
...
func create_payload_resize_window(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := resize_window_json_type{}
	if len(args) != 0 { return fmt.Errorf("%s", "Unknown extra argument(s) supplied to resize_window") }
payload.Match = escaped_string(options_resize_window.Match)
payload.Self = options_resize_window.Self
payload.Increment = options_resize_window.Increment
payload.Axis = options_resize_window.Axis
	io_data.rc.Payload = payload
	return
}

func create_rc_resize_window(args []string) (*utils.RemoteControlCmd, error) {
...
func run_resize_window(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_resize_window)
	if err != nil {
		return
	}

	rc, err := create_rc_resize_window(args)
	if err != nil {
		return
	}
	...
	io_data := rc_io_data{
		...
		string_response_is_err: true,
	}
	err = create_payload_resize_window(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_resize_window(parent *cli.Command) *cli.Command {
	ans := parent.AddSubCommand(&cli.Command{
		Name:             "resize-window",
		Usage:            "",
		ShortDescription: "Resize the specified windows",
		HelpText:         "Resize the specified windows in the current layout. Note that not all layouts can resize all windows in all directions.",
		Run:              run_resize_window,
	})
...
ans.Add(cli.OptionSpec{
            ...
            Type: "int",
            Dest: "Increment",
            Help: "The number of cells to change the size by, can be negative to decrease the size.",
...
ans.Add(cli.OptionSpec{
            ...
            Type: "choices",
            Dest: "Axis",
            Help: "The axis along which to resize. If :code:`horizontal`, it will make the window wider or narrower by the specified increment. If :code:`vertical`, it will make the window taller or shorter by the specified increment. The special value :code:`reset` will reset the layout to its default configuration.",
        
Choices: "horizontal, reset, vertical",

Completer: cli.NamesCompleter("Choices for axis", "horizontal", "reset", "vertical"),
	Default: "horizontal",
})
ans.Add(cli.OptionSpec{
            Name: "--self",
...
func init() {
	register_at_cmd(setup_resize_window)
}
