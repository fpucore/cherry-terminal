...
type options_close_tab_type struct {
	Match string
Self bool
IgnoreNoMatch bool
}

var options_close_tab options_close_tab_type

type close_tab_json_type struct {
	Match escaped_string`json:"match,omitempty"`
Self bool`json:"self,omitempty"`
Ignore_no_match bool`json:"ignore_no_match,omitempty"`
}

func create_payload_close_tab(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := close_tab_json_type{}
	if len(args) != 0 { return fmt.Errorf("%s", "Unknown extra argument(s) supplied to close_tab") }
payload.Match = escaped_string(options_close_tab.Match)
payload.Self = options_close_tab.Self
payload.Ignore_no_match = options_close_tab.IgnoreNoMatch
	io_data.rc.Payload = payload
	return
}

func create_rc_close_tab(args []string) (*utils.RemoteControlCmd, error) {
...
func run_close_tab(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_close_tab)
	if err != nil {
		return
	}

	rc, err := create_rc_close_tab(args)
	if err != nil {
		return
	}
	...
	err = create_payload_close_tab(&io_data, cmd, args)
	if err != nil {
		return
	}
...
ans.Add(cli.OptionSpec{
            ...
            Type: "bool-set",
            Dest: "Self",
            Help: "Close the tab of the window this command is run in, rather than the active tab.",
        })
ans.Add(cli.OptionSpec{
            Name: "--ignore-no-match",
            Type: "bool-set",
            Dest: "IgnoreNoMatch",
            Help: "Do not return an error if no tabs are matched to be closed.",
        })
	return ans
}

func init() {
	register_at_cmd(setup_close_tab)
}
