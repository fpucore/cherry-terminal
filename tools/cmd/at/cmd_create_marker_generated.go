...
type options_create_marker_type struct {
	Match string
Self bool
}

var options_create_marker options_create_marker_type

type create_marker_json_type struct {
	Match escaped_string`json:"match,omitempty"`
Self bool`json:"self,omitempty"`
Marker_spec []escaped_string`json:"marker_spec,omitempty"`
}

func create_payload_create_marker(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := create_marker_json_type{}
	if len(args) < 2 { return fmt.Errorf("%s", "Must specify at least 2 arguments to create_marker") }
payload.Marker_spec = escape_list_of_strings(args)
payload.Match = escaped_string(options_create_marker.Match)
payload.Self = options_create_marker.Self
	io_data.rc.Payload = payload
	return
}

func create_rc_create_marker(args []string) (*utils.RemoteControlCmd, error) {
...
func run_create_marker(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_create_marker)
	if err != nil {
		return
	}

	rc, err := create_rc_create_marker(args)
	if err != nil {
		return
	}
	...
	err = create_payload_create_marker(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_create_marker(parent *cli.Command) *cli.Command {
	ans := parent.AddSubCommand(&cli.Command{
		Name:             "create-marker",
		Usage:            " MARKER SPECIFICATION",
		ShortDescription: "Create a marker that highlights specified text",
		HelpText:         "Create a marker which can highlight text in the specified window. For example: :code:`create_marker text 1 ERROR`. For full details see: :doc:`marks`",
		Run:              run_create_marker,
	})
...
func init() {
	register_at_cmd(setup_create_marker)
}
