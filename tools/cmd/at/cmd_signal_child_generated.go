...
type options_signal_child_type struct {
	Match string
}

var options_signal_child options_signal_child_type

type signal_child_json_type struct {
	Signals []escaped_string`json:"signals,omitempty"`
Match escaped_string`json:"match,omitempty"`
}

func create_payload_signal_child(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := signal_child_json_type{}
	if len(args) == 0 {
args = append(args, "SIGINT")
}
payload.Signals = escape_list_of_strings(args)
payload.Match = escaped_string(options_signal_child.Match)
	io_data.rc.Payload = payload
	return
}

func create_rc_signal_child(args []string) (*utils.RemoteControlCmd, error) {
...
func run_signal_child(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_signal_child)
	if err != nil {
		return
	}

	rc, err := create_rc_signal_child(args)
	if err != nil {
		return
	}
	...
	err = create_payload_signal_child(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_signal_child(parent *cli.Command) *cli.Command {
	ans := parent.AddSubCommand(&cli.Command{
		Name:             "signal-child",
		Usage:            " [SIGNAL_NAME ...]",
		ShortDescription: "Send a signal to the foreground process in the specified windows",
		HelpText:         "Send one or more signals to the foreground process in the specified windows. If you use the :option:`kitten @ signal-child --match` option the signal will be sent for all matched windows. By default, only the active window is affected. If you do not specify any signals, :code:`SIGINT` is sent by default. You can also map :ac:`signal_child` to a shortcut in :file:`kitty.conf`, for example::\n\n    map f1 signal_child SIGTERM",
		Run:              run_signal_child,
	})
...
func init() {
	register_at_cmd(setup_signal_child)
}
