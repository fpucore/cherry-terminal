...
type options_set_spacing_type struct {
	All bool
Configured bool
Match string
MatchTab string
...
var options_set_spacing options_set_spacing_type

type set_spacing_json_type struct {
...
func create_payload_set_spacing(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := set_spacing_json_type{}
	if len(args) < 1 { return fmt.Errorf("%s", "Must specify at least one argument to set_spacing") }
payload.Settings, err = parse_set_spacing(args)
if err != nil { return err }
payload.Match_tab = escaped_string(options_set_spacing.MatchTab)
payload.All = options_set_spacing.All
payload.Configured = options_set_spacing.Configured
payload.Match_window = escaped_string(options_set_spacing.Match)
	io_data.rc.Payload = payload
	return
}

func create_rc_set_spacing(args []string) (*utils.RemoteControlCmd, error) {
...
func run_set_spacing(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_set_spacing)
	if err != nil {
		return
	}

	rc, err := create_rc_set_spacing(args)
	if err != nil {
		return
	}
	...
	err = create_payload_set_spacing(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_set_spacing(parent *cli.Command) *cli.Command {
	...
	ans.Add(cli.OptionSpec{
            ...
            Type: "bool-set",
            Dest: "All",
            Help: "By default, settings are only changed for the currently active window. This option will cause paddings and margins to be changed in all windows.",
        })
ans.Add(cli.OptionSpec{
            Name: "--configured -c",
            Type: "bool-set",
            Dest: "Configured",
            Help: "Also change the configured paddings and margins (i.e. the settings kitty will use for new windows).",
        })
ans.Add(cli.OptionSpec{
            Name: "--match -m",
            Type: "",
...
func init() {
	register_at_cmd(setup_set_spacing)
}
