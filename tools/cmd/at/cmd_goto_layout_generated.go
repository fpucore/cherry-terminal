...
type options_goto_layout_type struct {
	Match string
}

var options_goto_layout options_goto_layout_type

type goto_layout_json_type struct {
	Layout escaped_string`json:"layout,omitempty"`
Match escaped_string`json:"match,omitempty"`
}

func create_payload_goto_layout(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := goto_layout_json_type{}
	if len(args) != 1 { return fmt.Errorf("%s", "Must specify exactly 1 argument(s) for goto_layout") }
payload.Layout = escaped_string(args[0])
payload.Match = escaped_string(options_goto_layout.Match)
	io_data.rc.Payload = payload
	return
}

func create_rc_goto_layout(args []string) (*utils.RemoteControlCmd, error) {
...
func run_goto_layout(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_goto_layout)
	if err != nil {
		return
	}

	rc, err := create_rc_goto_layout(args)
	if err != nil {
		return
	}
	...
	err = create_payload_goto_layout(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func init() {
	register_at_cmd(setup_goto_layout)
}
