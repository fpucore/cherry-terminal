...
type options_scroll_window_type struct {
	Match string
}

var options_scroll_window options_scroll_window_type

type scroll_window_json_type struct {
	Amount []any`json:"amount,omitempty"`
Match escaped_string`json:"match,omitempty"`
}

func create_payload_scroll_window(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := scroll_window_json_type{}
	if len(args) != 1 { return fmt.Errorf("%s", "Must specify exactly 1 argument(s) for scroll_window") }
payload.Amount, err = parse_scroll_amount(args[0])
if err != nil { return err }
payload.Match = escaped_string(options_scroll_window.Match)
	io_data.rc.Payload = payload
	return
}

func create_rc_scroll_window(args []string) (*utils.RemoteControlCmd, error) {
...
func run_scroll_window(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_scroll_window)
	if err != nil {
		return
	}

	rc, err := create_rc_scroll_window(args)
	if err != nil {
		return
	}
	...
	err = create_payload_scroll_window(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func init() {
	register_at_cmd(setup_scroll_window)
}
