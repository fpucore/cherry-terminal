...
var options_ls options_ls_type

type ls_json_type struct {
	All_env_vars bool`json:"all_env_vars,omitempty"`
Match escaped_string`json:"match,omitempty"`
Match_tab escaped_string`json:"match_tab,omitempty"`
Self bool`json:"self,omitempty"`
Output_format escaped_string`json:"output_format,omitempty"`
}

func create_payload_ls(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := ls_json_type{}
	if len(args) != 0 { return fmt.Errorf("%s", "Unknown extra argument(s) supplied to ls") }
payload.All_env_vars = options_ls.AllEnvVars
payload.Match = escaped_string(options_ls.Match)
payload.Match_tab = escaped_string(options_ls.MatchTab)
payload.Self = options_ls.Self
payload.Output_format = escaped_string(options_ls.OutputFormat)
	io_data.rc.Payload = payload
	return
}

func create_rc_ls(args []string) (*utils.RemoteControlCmd, error) {
...
func run_ls(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_ls)
	if err != nil {
		return
	}

	rc, err := create_rc_ls(args)
	if err != nil {
		return
	}
	...
	err = create_payload_ls(&io_data, cmd, args)
	if err != nil {
		return
	}
...
ans.Add(cli.OptionSpec{
            ...
            Type: "bool-set",
            Dest: "AllEnvVars",
            Help: "Show all environment variables in output, not just differing ones.",
        })
ans.Add(cli.OptionSpec{
            Name: "--self",
            Type: "bool-set",
            Dest: "Self",
            Help: "Only list the window this command is run in.",
        })
ans.Add(cli.OptionSpec{
            Name: "--output-format",
            Type: "choices",
            Dest: "OutputFormat",
            Help: "Output in JSON or kitty session format",
        
Choices: "json, session",

Completer: cli.NamesCompleter("Choices for output-format", "json", "session"),
	Default: "json",
})
ans.Add(cli.OptionSpec{
            Name: "--match -m",
