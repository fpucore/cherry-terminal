...
type options_send_key_type struct {
	Match string
MatchTab string
All bool
ExcludeActive bool
...
var options_send_key options_send_key_type

type send_key_json_type struct {
	Keys []escaped_string`json:"keys,omitempty"`
...
func create_payload_send_key(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := send_key_json_type{}
	payload.Keys = escape_list_of_strings(args)
payload.Match = escaped_string(options_send_key.Match)
payload.Match_tab = escaped_string(options_send_key.MatchTab)
payload.All = options_send_key.All
payload.Exclude_active = options_send_key.ExcludeActive
	io_data.rc.Payload = payload
	return
}

func create_rc_send_key(args []string) (*utils.RemoteControlCmd, error) {
...
func run_send_key(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_send_key)
	if err != nil {
		return
	}

	rc, err := create_rc_send_key(args)
	if err != nil {
		return
	}
	...
	err = create_payload_send_key(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func init() {
	register_at_cmd(setup_send_key)
}
