...
type options_action_type struct {
	Self bool
Match string
}

var options_action options_action_type

type action_json_type struct {
	Action escaped_string`json:"action,omitempty"`
Match_window escaped_string`json:"match_window,omitempty"`
Self bool`json:"self,omitempty"`
}

func create_payload_action(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := action_json_type{}
	if len(args) < 1 { return fmt.Errorf("%s", "Must specify at least one argument to action") }
payload.Action = escaped_string(strings.Join(args, " "))
payload.Self = options_action.Self
payload.Match_window = escaped_string(options_action.Match)
	io_data.rc.Payload = payload
	return
}

func create_rc_action(args []string) (*utils.RemoteControlCmd, error) {
...
func run_action(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_action)
	if err != nil {
		return
	}

	rc, err := create_rc_action(args)
	if err != nil {
		return
	}
	...
	err = create_payload_action(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_action(parent *cli.Command) *cli.Command {
	ans := parent.AddSubCommand(&cli.Command{
		Name:             "action",
		Usage:            " ACTION [ARGS FOR ACTION...]",
		ShortDescription: "Run the specified mappable action",
		HelpText:         "Run the specified mappable action. For a list of all available mappable actions, see :doc:`actions`. Any arguments for ACTION should follow the action. Note that parsing of arguments is action dependent so for best results specify all arguments as single string on the command line in the same format as you would use for that action in kitty.conf.",
		Run:              run_action,
	})
	ans.ArgCompleter = complete_actions
ans.Add(cli.OptionSpec{
            Name: "--self",
            Type: "bool-set",
            Dest: "Self",
            Help: "Run the action on the window this command is run in instead of the active window.",
        })
ans.Add(cli.OptionSpec{
            Name: "--no-response",
            Type: "bool-set",
