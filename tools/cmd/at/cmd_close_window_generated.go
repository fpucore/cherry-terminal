...
type options_close_window_type struct {
	Match string
Self bool
IgnoreNoMatch bool
}

var options_close_window options_close_window_type

type close_window_json_type struct {
	Match escaped_string`json:"match,omitempty"`
Self bool`json:"self,omitempty"`
Ignore_no_match bool`json:"ignore_no_match,omitempty"`
}

func create_payload_close_window(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := close_window_json_type{}
	if len(args) != 0 { return fmt.Errorf("%s", "Unknown extra argument(s) supplied to close_window") }
payload.Match = escaped_string(options_close_window.Match)
payload.Self = options_close_window.Self
payload.Ignore_no_match = options_close_window.IgnoreNoMatch
	io_data.rc.Payload = payload
	return
}

func create_rc_close_window(args []string) (*utils.RemoteControlCmd, error) {
...
func run_close_window(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_close_window)
	if err != nil {
		return
	}

	rc, err := create_rc_close_window(args)
	if err != nil {
		return
	}
	...
	err = create_payload_close_window(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_close_window(parent *cli.Command) *cli.Command {
	ans := parent.AddSubCommand(&cli.Command{
		Name:             "close-window",
		Usage:            "",
		ShortDescription: "Close the specified windows",
		HelpText:         "Close the specified windows",
		Run:              run_close_window,
	})
...
ans.Add(cli.OptionSpec{
            ...
            Type: "bool-set",
            Dest: "Self",
            Help: "Close the window this command is run in, rather than the active window.",
        })
ans.Add(cli.OptionSpec{
            Name: "--ignore-no-match",
            Type: "bool-set",
            Dest: "IgnoreNoMatch",
            Help: "Do not return an error if no windows are matched to be closed.",
        })
	return ans
}

func init() {
	register_at_cmd(setup_close_window)
}
