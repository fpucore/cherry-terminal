...
type options_disable_ligatures_type struct {
	All bool
Match string
MatchTab string
}

var options_disable_ligatures options_disable_ligatures_type

type disable_ligatures_json_type struct {
...
func create_payload_disable_ligatures(io_data *rc_io_data, cmd *cli.Command, args []string) (err error) {
	payload := disable_ligatures_json_type{}
	if len(args) != 1 { return fmt.Errorf("%s", "Must specify exactly 1 argument(s) for disable_ligatures") }
if len(args) != 1 { return fmt.Errorf("%s", "Must specify exactly 1 argument for disable_ligatures") }
switch(args[0]) {
case "never", "always", "cursor":
	payload.Strategy = args[0]
default: return fmt.Errorf("%s is not a valid choice. Allowed values: %s", args[0], `"never", "always", "cursor"`)
}
payload.Match_tab = escaped_string(options_disable_ligatures.MatchTab)
payload.All = options_disable_ligatures.All
payload.Match_window = escaped_string(options_disable_ligatures.Match)
	io_data.rc.Payload = payload
	return
}

func create_rc_disable_ligatures(args []string) (*utils.RemoteControlCmd, error) {
	rc := utils.RemoteControlCmd{
		Cmd:        "disable-ligatures",
...
func run_disable_ligatures(cmd *cli.Command, args []string) (return_code int, err error) {
	err = cmd.GetOptionValues(&options_disable_ligatures)
	if err != nil {
		return
	}

	rc, err := create_rc_disable_ligatures(args)
	if err != nil {
		return
	}
	...
	err = create_payload_disable_ligatures(&io_data, cmd, args)
	if err != nil {
		return
	}
...
func setup_disable_ligatures(parent *cli.Command) *cli.Command {
	ans := parent.AddSubCommand(&cli.Command{
		Name:             "disable-ligatures",
		Usage:            " STRATEGY",
		ShortDescription: "Control ligature rendering",
		HelpText:         "Control ligature rendering for the specified windows/tabs (defaults to active window). The :italic:`STRATEGY` can be one of: :code:`never`, :code:`always`, :code:`cursor`.",
		Run:              run_disable_ligatures,
	})
...
ans.Add(cli.OptionSpec{
            ...
            Type: "bool-set",
            Dest: "All",
            Help: "By default, ligatures are only affected in the active window. This option will cause ligatures to be changed in all windows.",
        })
ans.Add(cli.OptionSpec{
            Name: "--match -m",
            Type: "",
...
func init() {
	register_at_cmd(setup_disable_ligatures)
}
